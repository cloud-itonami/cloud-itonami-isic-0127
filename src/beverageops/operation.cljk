(ns beverageops.operation
  "OperationActor -- one beverage-crop-plantation operation = one
  supervised actor run. The advisor (BeverageOpsAdvisor) is sealed into a
  single node (:advise); its proposal is ALWAYS routed through the
  Beverage Operations Governor (:govern) and the rollout phase gate
  ...
  One run = one plantation coordination operation (intake -> advise ->
  govern -> decide -> commit | hold | approval). No unbounded inner loop
  -- each operation is auditable and checkpointed. A plantation/block's
  operating history is advanced by MANY operations (log-plantation-record
  ...
  hands the decision to a human operator (grower / plantation manager /
  ...
  Governor is clean -- see `beverageops.governor/always-escalate-ops`.
  ...
  (:require [beverageops.advisor :as advisor]
...
(defn- commit-record [request _context proposal]
  {:effect  (:effect proposal)
   :path    [(:plantation-id request)]
   :value   (or (:value proposal) {})
   :payload (:value proposal)})

(defn run-operation
  "Run one plantation operation through the advisor -> governor -> phase
  ...
  (let [;; Step 1: Advisor proposes
        ...
        disposition-fact (case disposition
                           ...
                           :escalate {:t :approval-requested
                                      :op (:op request) :subject (:plantation-id request)
                                      :reason (or reason
